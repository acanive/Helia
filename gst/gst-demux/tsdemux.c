    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_PROGRAM_NUMBER:
      ...
      break;
	case PROP_PROG_NAME:
	  if ( demux->prog_name ) g_free ( demux->prog_name );
	  demux->prog_name = g_strdup ( g_value_get_string (value) );
...
#if USE_CSASYS
	  sys_get_key ( demux->prog_name, demux->requested_program_number, demux->cw );
#endif
	  break;
...
#if USE_OSCAM
	if ( packet->scrambling )
		demux_data ( demux->prog_name, demux->requested_program_number, packet->data, size );
#endif

#if USE_CSASYS
	if ( packet->scrambling )
		sys_set_key ( demux->cw, packet->data, size );
#endif
