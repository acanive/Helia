...
static void base_create_player ( Base *base, GtkBox *box )
{
	...
	base->player->treeview = create_treeview ( base, _i18n_ ( base, "Files" ), TRUE );
...
static void base_create_dtv ( Base *base, GtkBox *box )
{
	...
	base->dtv->treeview = create_treeview ( base, _i18n_ ( base, "Channels" ), FALSE );
...
static void base_action_widget_visible ( GtkWidget *widget )
{
...
static void base_action_search ( G_GNUC_UNUSED GSimpleAction *sl, G_GNUC_UNUSED GVariant *pm, Base *base )
{
	if ( gtk_widget_get_visible ( GTK_WIDGET ( mp_vbox ) ) )
	{
		if ( gtk_widget_get_visible ( GTK_WIDGET ( base->player->vbox_sw_mp ) ) )
			base_action_widget_visible ( GTK_WIDGET ( base->player->search_entry ) );
	}

	if ( gtk_widget_get_visible ( GTK_WIDGET ( tv_vbox ) ) )
	{
		if ( gtk_widget_get_visible ( GTK_WIDGET ( base->dtv->vbox_sw_tv ) ) )
			base_action_widget_visible ( GTK_WIDGET ( base->dtv->search_entry ) );
	}
}
...
static void base_app_quit ( G_GNUC_UNUSED GtkWindow *win, Base *base )
{
	...
	gtk_entry_set_text ( base->dtv->search_entry, "" );
...
static void base_win ( GtkApplication *app, GFile **files, int n_files )
{
	...
	gtk_window_set_title ( base->window, "Media Player & Digital TV");
	gtk_window_set_default_size ( base->window, base->win_width, base->win_height );
	...
	if ( base->win_height < 400 ) { size_b = 192; size_c = 40; }
	if ( base->win_height < 300 ) { size_b = 128; size_c = 32; }
	if ( base->win_height < 200 ) { size_b = 64;  size_c = 24; }

	base_create_image_button      ( bt_hbox, "helia-mp", size_b, base_set_win_mp, base );
	base_create_image_flip_button ( bt_hbox, "helia-tv", size_b, base_set_win_tv, base );
	...
	base_create_image_button ( bc_hbox, "helia-pref", size_c, base_pref,  base );
	base_create_image_button ( bc_hbox, "helia-keyb", size_c, base_keyb,  base );
	base_create_image_button ( bc_hbox, "helia-info", size_c, base_info,  base );
	base_create_image_button ( bc_hbox, "helia-quit", size_c, base_close, base );
	...
	gtk_widget_hide ( GTK_WIDGET ( base->player->search_entry ) );
	gtk_widget_hide ( GTK_WIDGET ( base->dtv->search_entry ) );

	gtk_widget_set_opacity ( GTK_WIDGET ( base->window ), base->opacity_win );
	gtk_window_resize ( base->window, base->win_width, base->win_height );
