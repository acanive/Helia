...
{
	GtkTreeModel *model_filter = gtk_tree_view_get_model ( base->dtv->scan.treeview_scan );
    GtkTreeModel *model = gtk_tree_model_filter_get_model ( GTK_TREE_MODEL_FILTER ( model_filter ) );
...
static GtkBox * scan_channels ( Base *base )
{
	...
	base->dtv->scan.treeview_scan = create_treeview ( base, _i18n_ ( base, "Channels" ), FALSE );
...
void scan_win_create ( Base *base )
{
	...
	for ( j = 0; j < PAGE_NUM; j++ )
	{
		...
		gtk_notebook_append_page ( notebook, GTK_WIDGET ( m_box_n[j] ),  gtk_label_new ( _i18n_ ( base, scan_label_n[j].name ) ) );
...
char * helia_get_dvb_info ( Base *base, uint adapter, uint frontend )
{
	...
	if ( ( fd = g_open ( fd_name, flags ) ) == -1 )
	{
		...
		if ( ( fd = g_open ( fd_name, flags ) ) == -1 )
		{
			...
			return g_strdup ( _i18n_ ( base, "Undefined" ) );
		}
	}
