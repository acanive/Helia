...
static void treeview_up_down ( GtkTreeView *tree_view, gboolean up_dw )
{
	...
	/*else if ( gtk_tree_model_get_iter_first ( model, &child_iter ) )
	{
		gtk_tree_selection_select_iter ( gtk_tree_view_get_selection (tree_view), &child_iter );
	}*/
}
...
GtkBox * treeview_box ( Base *base, G_GNUC_UNUSED GtkWindow *window, GtkTreeView *tree_view, GtkBox *box, gboolean player_tv )
{
	...
	const struct PanelTrwNameIcon { const char *name; void (* activate)(); } NameIcon_n[] =
	{
		...
		{ ( player_tv ) ? "helia-add-folder" : "helia-display", ( player_tv ) ? treeview_open_dir : treeview_scan },
		{ ( player_tv ) ? "helia-repeat"     : "helia-window", 	( player_tv ) ? treeview_repeat   : treeview_ve   },
...
static void treeview_add_m3u ( Base *base, const char *file )
{
    ...
    gboolean first = TRUE, play = FALSE;
        ...
        //for ( i = 0; lines[i] != NULL && *lines[i]; i++ )
        {
			...
			if ( g_str_has_prefix ( lines[i], "#EXTINF" ) )
			{
				char **lines_info = g_strsplit ( lines[i], ",", 0 );
					...
					if ( first && ( g_str_has_prefix ( lines[i+1], "/" ) || g_str_has_prefix ( lines[i+1], "file://") ) ) play = TRUE;

					treeview_append ( base->player->treeview, base, play, 
			...
			else
			{
				...
				if ( first && ( g_str_has_prefix ( lines[i], "/" ) || g_str_has_prefix ( lines[i], "file://") ) ) play = TRUE;
			...
			if ( play ) { first = FALSE; play = FALSE; }
        }
...
static void create_columns ( GtkTreeView *tree_view, const char *name, int column_id, gboolean col_vis )
{
...
static void add_columns ( GtkTreeView *tree_view, const char *title )
{
    ...
    {
        create_columns ( tree_view, col_title_list_n[c].title, c, col_title_list_n[c].vis );
	}
}
...
static gboolean search_entry_row_mp_visible ( GtkTreeModel *model, GtkTreeIter *iter, Base *base )
{
	gboolean visible = FALSE;
    
    const char *text = gtk_entry_get_text ( base->player->search_entry );
    
    if ( gtk_entry_get_text_length ( base->player->search_entry ) > 0 )
...
static gboolean search_entry_row_tv_visible ( GtkTreeModel *model, GtkTreeIter *iter, Base *base )
{
	gboolean visible = FALSE;
    
    const char *text = gtk_entry_get_text ( base->dtv->search_entry );
    
    if ( gtk_entry_get_text_length ( base->dtv->search_entry ) > 0 )
		visible = search_entry_row_all_visible ( model, iter, text );
	else
		visible = TRUE;
...
GtkTreeView * create_treeview ( Base *base, const char *title, gboolean player_tv )
{
	...
	if ( player_tv )
		gtk_tree_model_filter_set_visible_func ( filtered, (GtkTreeModelFilterVisibleFunc)search_entry_row_mp_visible, base, NULL );
	else
		gtk_tree_model_filter_set_visible_func ( filtered, (GtkTreeModelFilterVisibleFunc)search_entry_row_tv_visible, base, NULL );
